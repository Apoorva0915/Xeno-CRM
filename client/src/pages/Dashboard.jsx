...
const Dashboard = () => {
  ...
  return (
    <div className="md:p-10 bg-gray-100">
      <h1 className="text-4xl font-bold text-[#002d9c] mb-10">Dashboard</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2  lg:grid-cols-3 gap-8">
        {dashboardItems.map((item, index) => (
          <Link
            key={index}
            to={item.path}
            className="group bg-white p-6 rounded-2xl hover:bg-indigo-500 shadow-md hover:shadow-lg transition transform hover:-translate-y-1 cursor-pointer"
          >
            ...
            <h2 className="text-lg font-semibold text-center text-gray-700 group-hover:text-white  transition-colors">
              {item.label}
            </h2>
          </Link>
        ))}
